...
- `docs/requirements_specification.md` and `docs/design_specification.md` share one status vocabulary — `Draft`, `Accepted`, `Rejected`, `Obsolete` — documented in each file's own status section. Every requirement and decision is currently `Accepted`. A `Rejected` or `Obsolete` item is never deleted; it keeps its text plus a `Comment` field (immediately after `Status`) explaining why. Whether downstream `ADS-*`/`TC-*` linkage must gate on `Accepted` status is still undecided — don't invent that rule; ask first.
- A new design decision should be `Accepted` unless you have a concrete reason to mark it `Draft` pending review — check `design_specification.md` Section 10's traceability summary counts and update them if you add/remove decisions.
