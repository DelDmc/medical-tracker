...
**FR-003** — The application shall reject registration when the password is missing or does not meet the accepted minimum-length policy.
**Status:** Accepted
**Verification:** An API validation test shall confirm that registration is rejected when the password is missing or shorter than the accepted minimum length.
